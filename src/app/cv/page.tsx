...
export default function CVPage() {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground pb-20">
      {/* LE DOUBLON A ÉTÉ SUPPRIMÉ ICI */}

      <main className="container max-w-5xl mx-auto px-4 pt-20 md:pt-24">
        
        {/* Barre d'action pour le CV */}
        <div className="flex justify-between items-center mb-10 border-b border-white/5 pb-6">
          <h1 className="text-3xl font-bold tracking-tighter">{content.title}</h1>
          
          {/* MODIFICATION ICI : Utilisation d'un lien pour le téléchargement */}
          <a 
            href={content.cvFile} 
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
          ...
          <div className="space-y-10">
            <section>
              <h2 className="text-lg font-bold mb-4 flex items-center gap-2 uppercase tracking-widest text-primary/80">
          ...
          <div className="lg:col-span-2 space-y-16">
            <section>
              <h2 className="text-xl font-bold mb-8 flex items-center gap-3 border-l-4 border-primary pl-4">
                <Briefcase className="h-5 w-5 text-primary" /> {content.experienceTitle}
              </h2>
              <div className="space-y-10">
                {content.experiences.map((exp, i) => (
                  <div key={i} className="relative pl-8 border-l border-white/10">
                    <div className="absolute -left-[5px] top-1.5 w-2.5 h-2.5 rounded-full bg-primary shadow-[0_0_10px_rgba(var(--primary),0.5)]" />
                    <div className="flex flex-wrap justify-between items-start gap-2 mb-2">
                      <h3 className="font-bold text-lg leading-tight">{exp.role}</h3>
                      <span className="text-[10px] font-mono bg-white/5 border border-white/10 px-2 py-1 rounded text-primary uppercase tracking-tighter">
                        {exp.date}
                      </span>
                    </div>
                    <p className="text-primary/90 font-semibold text-sm mb-3">{exp.company}</p>
                ...
                {content.education.map((edu, i) => (
                  <div key={i} className="relative pl-8 border-l border-white/10">
                    ...
                    <div className="flex flex-wrap justify-between items-start gap-2 mb-2">
                      ...
                      <span className="text-[10px] font-mono text-muted-foreground uppercase tracking-tighter">{edu.date}</span>
                    </div>
                    <p className="text-primary/90 font-semibold text-sm mb-3">{edu.school}</p>
